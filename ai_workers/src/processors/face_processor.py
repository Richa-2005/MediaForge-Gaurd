...
def process_faces(image_path: Path, output_dir: Path):
    image = load_image(image_path)
    boxes = detect_faces(image)
    faces = crop_faces(image, boxes)
    saved_paths = save_faces(faces, output_dir)
...
if __name__ == "__main__":
    ...
    demo_dir = Path("outputs/demo_faces")

    result = process_faces(sample_image, demo_dir)
